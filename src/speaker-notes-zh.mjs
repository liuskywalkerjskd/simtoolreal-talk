const note=(title,zhTitle,timing,script,cue='')=>({title,zhTitle,timing,script,cue});

export const zhNotes=[
note('SimToolReal','SimToolReal：从工具运动到机器人技能','约 50 秒',
...
'先只显示问题，让听众回答；随后按三次前进键依次出现三条路线。'),

note('What makes demonstration-based tool use difficult?','为什么工具使用中的示范学习更困难？','约 100 秒',
`这页用三个具体画面说明两种瓶颈。先看左侧：为了采集灵巧手的机器人动作示范，这个系统同时使用 wrist camera、头戴设备、手套和控制器。它不代表所有遥操作系统都必须这么复杂，但直观说明了高质量灵巧示范往往伴随较重的硬件、标定和操作负担。

再看右侧同一把锤子。人手能够形成稳定的环绕式抓握；把相同的抓握意图转移到灵巧手后，由于手指长度、关节范围和形态不同，这个姿态并不能形成可行抓握。也就是说，人类演示可以告诉我们“想怎样抓”，却不自动给出机器人能够稳定执行的动作。

工具使用又比普通 pick-and-place 更依赖接触质量，任务中还可能出现手内旋转、持续接触和外界反作用力。因此，动作看起来相似，并不等于接触在物理上成立。
...
'第一次揭示左侧遥操作设备；第二次揭示右侧人手与灵巧手对比；第三次出现底部结论。'),

note('Tool use as a sequence of pose-reaching problems','把工具使用转化为连续的位姿到达问题','约 90 秒',
...
'讲清楚 goal conditioning 与 policy training 的区别。'),

note('A 6D pose specifies position and orientation','6D 位姿同时描述位置与朝向','约 90 秒',
...
'依次播放 lift、reorient、swing 三个构建步骤，让听众先观察位置变化，再观察朝向变化。'),

note('Training pipeline: learn pose reaching through interaction','仿真训练 Pipeline：通过交互学习位姿到达','约 120 秒',
...
'指出图中的小工具只是分布示意，不是论文资产的逐一复刻。'),

note('RL training signal: how each reward term is constructed','RL 训练信号：每个 Reward 如何构造','约 180 秒',
...
'前两次前进键展开 t 和 t+1；第三次解释 cell state；第四次解释 hidden state；第五次强调它们是历史摘要，而不是原始历史帧堆叠。'),

note('The action head controls the arm and hand differently','Action Head 对手臂与手指采用不同控制方式','约 120 秒',
...
'先讲 simulator 一侧，再 reveal real robot 一侧与底部过渡句。'),

note('Real deployment supplies the same policy interface','真实部署 Pipeline：向固定策略提供同样的接口','约 120 秒',
...
'按 setup、goal extraction、online tracking、fixed actor 的顺序走完整张图。'),

note('The robot advances goals only after reaching them','目标切换由到达事件触发，而不是按演示时钟播放','约 90 秒',
...
note('Structured simulation as a route to reusable robot skills','总结与思考：结构化仿真及其两个延伸方向','约 210 秒',
...
'主结论直接出现；第一次前进键展示 task interface，第二次展示轻量级 VLM。最后用页面底部的边界说明收束。'),
